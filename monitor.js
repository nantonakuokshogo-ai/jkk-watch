...
async function enterFromHome(page) {
  ...
  const link = page.locator('a[href*="akiyaJyoukenStartInit"]');
  if (await link.count()) {
    console.log('[enter] click link to StartInit');
    await Promise.all([
...
// 中継「こちら」突破（href抽出→meta refresh→強制クリック）
async function passRelay(page) {
  ...
  const tryForceClick = async (ctx, label) => {
    ...
    if (await loc.count()) {
      console.log(`[relay] force click on ${label}`);
      try {
        await Promise.all([
  ...
  // main → frames の順で試す
  if (!(await tryParseHtmlForHref(page, "main"))) {
    for (const f of page.frames()) if (await tryParseHtmlForHref(f, `frame:${await f.url()}`)) break;
  }
  if (!(await tryMetaRefresh(page, "main"))) {
    for (const f of page.frames()) if (await tryMetaRefresh(f, `frame:${await f.url()}`)) break;
  }
...
// ===== メイン =====
(async () => {
  const browser = await chromium.launch({ he
